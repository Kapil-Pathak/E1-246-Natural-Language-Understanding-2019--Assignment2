...
corpus=open('/home/kapil/Hindi_Translation/additive/eng_hindi.txt', 'r').readlines()
corpus=corpus[:10000]
print(len(corpus))
...
for parallel in corpus:
    ...
    normalized_ta = normalize_string_h(ta).split()
...
#             attn_energies = attn_energies.masked_fill(encoder_maskings,float('-inf'))#-1e12) # PAD masking
    ...
    def forward(self, inputs, context, max_length, encoder_outputs, encoder_maskings=None, is_training=False):
        ...
        for i in range(max_length):
            ...
            context, alpha = self.Key_ValueAttention(hidden, encoder_outputs, encoder_maskings)
    ...
    def decode(self, context, encoder_outputs):
        ...
        while decoded.data.tolist()[0] != target2index['</s>']and i<max_len: # until </s>
            ...
            #print(hidden.shape)
            context, alpha = self.Key_ValueAttention(hidden, encoder_outputs,None)
...
MODEL_SAVE_PATH="/home/kapil/Hindi_Translation/key_value/encoder.pt"
#torch.save(decoder.state_dict(), MODEL_SAVE_PATH)
torch.save(encoder.state_dict(), MODEL_SAVE_PATH)
MODEL_SAVE_PATH1="/home/kapil/Hindi_Translation/key_value/decoder.pt"
#torch.save(decoder.state_dict(), MODEL_SAVE_PATH)
...
#     show_plot_visdom()
    plt.show()
    plt.savefig('LSTM_key_value.png')
    plt.close()

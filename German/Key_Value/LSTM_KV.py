...
corpus=open('/home/kapil/deu.txt', 'r').readlines()
...
class Decoder(nn.Module):
    def __init__(self, input_size, embedding_size, hidden_size, n_layers=1, dropout_p=0.1):
        ...
        self.attn_v = nn.Linear(self.hidden_size, self.hidden_size)
    def init_hidden(self,inputs):
    ...
    def init_weight(self):
        ...
        self.attn_v.weight = nn.init.xavier_uniform(self.attn_v.weight)
...
bleuscore_unigram=nltk.translate.bleu_score.corpus_bleu(target_reference, hypothesis,weights=(1,0,0,0))
bleuscore_4_gram=nltk.translate.bleu_score.corpus_bleu(target_reference, hypothesis,weights=(0.25,0.25,0.25,0.25))
print('Source :',' '.join([i for i in source_reference[0] if i not in ['</s>']]))
print('Truth : ',' '.join([index2target[i] for i in target_reference[0].data.tolist()[0] if i not in [2, 3]]))
print('Prediction : ',' '.join([i for i in hypothesis[0] if i not in ['</s>']]))
f= open("bleuscore.txt","w+")
f.write("Unigram Bleu Score "+str(float(bleuscore_unigram)))
f.write("4 gram Bleu Score "+str(float(bleuscore_4_gram)))

if USE_CUDA:
    attn = attn.cpu()
print("Saving attention diagram")
show_attention(source_reference[10], hypothesis[10], attn.data)
